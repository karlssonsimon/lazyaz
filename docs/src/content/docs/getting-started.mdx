...
`lazyaz --version` (or `-v`) prints the build version. There are no other
...
5. **Open the command palette** — press `Ctrl+P` to search all commands: open new tabs, switch themes, switch tenants, browse the activity overlay, and more. (Switching subscriptions has its own top-level shortcut, `Ctrl+S`.)
...
lazyaz opens directly into these tabs on launch. If no startup tabs are
...
[Configuration](/lazyaz/configuration/overview/) for all options.
