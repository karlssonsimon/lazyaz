...
| Action | Keys |
|---|---|
| Close preview | `h` / `Left` / `Esc` / `Backspace` |
| Scroll down | `j` / `Down` |
| Scroll up | `k` / `Up` |
| Go to bottom | `G` / `End` |
| Go to top | `gg` / `Home` |
...
| Action | Keys |
|---|---|
| Close preview | `Left` / `Esc` / `Backspace` |
| Scroll down | `Down` |
| Scroll up | `Up` |
| Go to bottom | `End` |
| Go to top | `Home` |
...
| Exit visual mode / clear marks | `Esc` |
| Swap visual anchor | `o` |
| Sort entities (entities pane) | `s` |
| Requeue received DLQ message(s) | `R` |
| Yank message body | `y` |
...
| Requeue received DLQ message(s) | `Ctrl+R` (in the DLQ messages pane; reloads subscriptions elsewhere) |
| Yank message body | `Ctrl+C` |
| Close message preview | `Left` / `Backspace` / `Esc` |
</TabItem>
</Tabs>

DLQ operations (requeue, move, complete, abandon) and the "Filter: only
with dead letters" entity filter are available through the action menu
after either peeking or receiving-with-lock. Destructive DLQ operations
(requeue, complete, move) open a confirmation prompt before executing —
`y` / `Enter` confirms, `n` / `Esc` cancels. See
...
| Paste secrets from clipboard | `p` |
...
| Paste secrets from clipboard | `Ctrl+V` |
| Toggle mark (secrets) | `Space` |
| Visual-line mode (secrets) | `Shift+Down` / `Shift+Up` |
</TabItem>
</Tabs>

The items column switches between **Secrets**, **Certificates**, and
**Keys** depending on the row picked in the Kind column. Yank, reveal, and
marking work on secrets; certificates and keys expose create / import /
...
| Cycle widget focus | `Tab` |
| Primary action (same as `o`) | `Enter` |
...
The stock files (`default.json`, `standard.json`) are copied to
`~/.config/lazyaz/keymaps/` only when missing — they are never regenerated
or overwritten, so edits you make to them survive restarts.
...
// ~/.config/lazyaz/keymaps/default.json
{
  ...
  "bindings": {
    ...
    "yank_blob_content": ["y"],
    "preview_back": ["h", "left", "esc", "backspace"],
...
// ~/.config/lazyaz/keymaps/standard.json
{
  ...
  "bindings": {
    ...
    "preview_back": ["left", "esc", "backspace"],
    "preview_next_focus": ["tab"],
    "preview_previous_focus": ["shift+tab"],
    "preview_down": ["down"],
    "preview_up": ["up"],
    "jump_bottom": ["end"],
    "jump_top_prefix": ["home"],
    "requeue_dlq": ["ctrl+r"],
    "yank_message_body": ["ctrl+c"],
    "toggle_dlq_filter": ["ctrl+l"],
    "message_back": ["left", "backspace", "esc"],
    "yank_secret": ["ctrl+c"],
    "reveal_secret": ["alt+r"],
    "paste_secrets": ["ctrl+v"],
    "inspect": ["ctrl+i"],
