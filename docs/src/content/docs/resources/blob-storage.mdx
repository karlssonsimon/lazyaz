...
Hierarchy mode loads up to 5,000 entries per folder level. When a folder
holds more, the pane title shows a trailing `+` (e.g. `Blobs (5,000+)`)
and the action menu (`a`) offers **Load 5000 more**, which appends the
next chunk without refetching what's already loaded. The entry disappears
once the listing is exhausted and the title shows the exact count.
...
| `gg` / `Home` | Jump to top (`Home` jumps in a single press) |
| `h` / `Left` / `Esc` / `Backspace` | Close preview |
...
rows you see, so blobs hidden by the filter are never marked.
...
| Load 5000 more | Append the next chunk of entries (shown when the last load hit the cap) |
...
| `h` / `Left` | Go back |
| `Backspace` | Up one folder (blobs pane, hierarchy mode), otherwise back one pane |
| `/` | Filter current list |
| `Ctrl+D` / `Ctrl+U` | Half-page scroll |
| `a` | Action menu |
