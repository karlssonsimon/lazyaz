...
Press `a` on the blobs pane and choose **Upload files...** to open the file
browser. Navigate with `j`/`k`/`h`/`l`, mark files or folders with `space` (or
`v` for visual-line range), and press `enter` to confirm.

Uploads go to the virtual folder you're currently browsing — navigate to the
target location in blob storage first, then start the upload.

Folder picks are recursive: `./data/2026/q1/report.csv` becomes
`<currentFolder>/data/2026/q1/report.csv`. Symlinks are not followed.
...
For storage accounts with **hierarchical namespace** (Data Lake Gen2), folders
are first-class resources rather than virtual prefixes, so additional actions
appear:

- **Create folder...**
- **Rename folder...** (on a folder)
- **Delete folder...** (on a folder, recursive)

All write operations open a confirmation or input prompt before they touch
the storage account. On HNS accounts renames are atomic via the Data Lake
path API; on non-HNS accounts a blob "rename" is implemented as a server-side
copy followed by a delete of the original.
...
| Delete folder... | Delete the current folder, recursive (HNS-enabled accounts only) |

### Containers pane

| Action | Description |
|---|---|
| Create container... | Create a new container in the focused storage account |
