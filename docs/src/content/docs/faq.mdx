...
Yes. From the secrets pane, press `a` and choose **Create secret...** to open
a two-field form (name + value). Submitting creates a new secret, or — if a
secret with that name already exists — adds a new version. The value field is
masked by default; press `Ctrl+R` to reveal while typing.

Other write operations (delete, disable, manual version pinning) aren't
supported yet.
