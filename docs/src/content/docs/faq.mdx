...
Yes. On the blobs pane, press `a` and choose **Upload files...** to open the
file browser. See [Blob Storage — Uploading](/lazyaz/resources/blob-storage/#uploading)
for the full flow.

### Can I rename or delete blobs and containers?

Yes. The action menu (`a`) exposes Rename, Delete, and bulk-delete operations
...
[Container, folder & blob operations](/lazyaz/resources/blob-storage/#container-folder--blob-operations)
for the full list.
...
Yes. Open the command palette (`Ctrl+P`) or new-tab picker (`Ctrl+T`) and
...
Navigate to a queue or topic subscription, drill into the Active/DLQ picker,
and select **DLQ**. Then:

- For **all** DLQ messages: while on the Active/DLQ picker, press `a` and choose
  **Requeue all DLQ messages (N)**. This drains the DLQ in batches.
- For **specific** messages: drill into the DLQ messages pane, press `a` and
  choose **Peek messages** to see what's in the DLQ. To actually modify state,
  open the action menu again and choose **Receive DLQ messages (with lock)** —
  this locks messages so they can be requeued, moved, or completed safely.
  Mark messages with `Space` or visual-line mode (`v`) before choosing
  **Requeue**, **Move to...**, or **Complete**.

Each destructive operation opens a confirmation prompt before executing —
...
Yes. From the secrets pane, press `a` and choose **Create secret...** to
...
The action menu also exposes **Add new version...** on the versions pane and
**Delete secret...** on the items pane (soft-delete — items move to the
vault's recovery bin). Disable and manual version pinning aren't supported
yet.

### Can I browse certificates and keys?

Yes. Each vault has a three-row **Kind** picker between the vault list and
the items list — Secrets, Certificates, Keys. Picking a row switches the
items column. The action menu adapts: **Create key...**, **Import
certificate...**, and per-kind delete entries appear when the matching kind
is active.
...
Yes. Edit `~/.config/lazyaz/keymaps/default.json` or create a new keymap file. The stock keymap files are only written when missing, so your edits survive restarts. See [Keymaps](/lazyaz/configuration/keymaps/) for the full reference.
