...
func main() {
	...
	server, err := sbrpc.NewServer(*socketPath, servicebus.NewService(cred), db)
	if err != nil {
		fmt.Fprintf(os.Stderr, "failed to start service bus daemon: %v\n", err)
		os.Exit(1)
	}
	defer server.Close()
	if err := server.Serve(); err != nil {
		fmt.Fprintf(os.Stderr, "service bus daemon stopped: %v\n", err)
		os.Exit(1)
	}
}
...
func openServerCacheDB(explicitPath string) *cache.DB {
	...
	if path == "" {
		var err error
		path, err = cache.DefaultServerDBPath("sb")
		if err != nil {
