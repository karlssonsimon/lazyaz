...
func (m Model) RenderOverlays(view string) string {
	...
	if m.SubOverlay.Active {
		view = ui.RenderSubscriptionOverlay(m.SubOverlay, closeHint, m.Cursor, m.Subscriptions, m.CurrentSub, m.Loading, m.LoadingStartedAt, m.Styles, &m.Keymap, m.Width, m.Height, view)
	}
	if !m.EmbeddedMode && m.ThemeOverlay.Active {
