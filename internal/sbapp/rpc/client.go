...
type SessionCreateResult struct {
	...
	State   sbcore.Snapshot `json:"state"`
}

type ActionInvokeResult struct {
	Action sbcore.ActionResult `json:"action"`
	State  sbcore.Snapshot     `json:"state"`
}
...
func (c *Client) CreateSession() (sbcore.Snapshot, error) {
	resp, err := c.Call(Request{ID: fmt.Sprintf("session-create-%d", time.Now().UnixNano()), Method: "session.create"})
	if err != nil {
		return sbcore.Snapshot{}, err
	}
	if !resp.OK {
		return sbcore.Snapshot{}, errors.New(resp.Error)
	}
...
func (c *Client) GetState() (sbcore.Snapshot, error) {
...
func (c *Client) InvokeAction(req sbcore.ActionRequest) (ActionInvokeResult, error) {
